...
def find_max_plus_l(matrix, i, j, base_k, prefix_sum_rectangle, largest_expanded_k):
    ...
    while low < high:
        mid = (low + high) // 2
        if check_center_element(matrix, i, j, mid, prefix_sum_rectangle) and check_all_arms(matrix, i, j, mid, prefix_sum_rectangle):
            low = mid + 1
        else:
            high = mid
...
def find_largest_expanded_plus(matrix, prefix_sum_rectangle):
    n, m = len(matrix), len(matrix[0])
    #left, up = compute_prefix_sums_n(matrix)
    #prefix_sum_left_to_right = build_prefix_sum_left_to_right(matrix)
    ...
    for i in range(n):
        for j in range(m):
            ...
            if min(n-i, i+1, m-j, j+1) <= largest_expanded_k:
                continue
            #if prefix_sum_left_to_right[i][j+1]  - prefix_sum_left_to_right[i][j-1]  == 1:
            #    continue
            if not is_center_possible_with_prefix(matrix, i, j, largest_expanded_k, prefix_sum_rectangle):
                continue
            #left_count = prefix_sum_left_to_right[i][j+1] 
    ...
    #print("Проверено точек: ", count_checked_dots)
    return largest_expanded_k
